...
import MainScript.QuesterEntry;
...
public final class EasyEntManipulator extends MethodProvider {
    ...
    public String interact(NPC n, String action) throws InterruptedException {
        ...
        if (map.isWithinRange(n, 6)) {
            ...
            MiniMapTileDestination m = new MiniMapTileDestination(this.client.bot, n.getPosition());
            mouse.click(m);
        }
    ...
    public String interact(GroundItem g, String action) throws InterruptedException {
        ...
        if (g.getPosition().distance(myPlayer()) <= 1) {
            status = "Interacting with " + g.getName();
            g.interact(action);
        }
        else if (g.getPosition().isOnMiniMap(this.client.bot)) {
            status = "Walking to " + g.getName();
            MiniMapTileDestination m = new MiniMapTileDestination(this.client.bot, g.getPosition());
            mouse.click(m);
        }
    ...
    public String interact(RS2Object o, String action) throws InterruptedException {
        ...
        if (o.isVisible()) {
            ...
            MiniMapTileDestination m = new MiniMapTileDestination(this.client.bot, o.getPosition());
            mouse.click(m);
        }
    ...
    protected void DebugLog(String message){
        if(QuesterEntry.VERBOSE){log(message);}
        return;
    }
